...
class KeyboardController : public KeyboardReportParser
{
    public:
        KeyboardController(USBHost &usb) : hostKeyboard(&usb), key(0), keyOem(0), modifiers(0)
        {
            ...
            // Enable NumLock when KeyboardController is constructed.
            this->HandleLockingKeys(&hostKeyboard, UHS_HID_BOOT_KEY_NUM_LOCK);

            // TODO: NumLock LED does not turn on automatically via above...
            // hostKeyboard.SetReport(0, 0, 2, 0, 1, &kbdLockingKeys.bLeds);
        };
